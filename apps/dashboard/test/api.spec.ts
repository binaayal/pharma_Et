...
describe('the dashboard API client', () => {
  ...
  describe('when the server refuses', () => {
    it('raises an ApiError that still knows it was a 401', async () => {
      ...
      await expect(api.sales('stale')).rejects.toMatchObject({
        status: 401,
        message: 'invalid or expired token',
      });
      await expect(api.sales('stale')).rejects.toBeInstanceOf(ApiError);
    });
    ...
    it('keeps the server\'s wording when there is some', async () => {
      ...
      await expect(api.salesSummary('tok', '2026-01-01', '2026-12-31')).rejects.toThrow(
        'that report is outside your scope',
      );
    });
    ...
    it('still fails usefully when the body is not JSON at all', async () => {
      ...
      await expect(api.sales('tok')).rejects.toMatchObject({ status: 502 });
      await expect(api.sales('tok')).rejects.toThrow('request failed (502)');
    });
  });
  ...
  describe('the platform surface is kept separate (BR-2.2)', () => {
    it('logs in without a tenant token', async () => {
      ...
      await api.platform.login('ops@example.com', 'pw');
    ...
    it('sends the platform token, never a tenant one, to platform routes', async () => {
      ok([]);
      await api.platform.tenants('platform-token');
  ...
  describe('recognising an ended session', () => {
    ...
    it('recognises a 401 from the API client', async () => {
      ...
      const caught = await api.sales('stale').catch((cause: unknown) => cause);
    ...
    it('does not mistake any other failure for an ended session', async () => {
      for (const status of [400, 403, 404, 429, 500, 502]) {
        ...
        const caught = await api.sales('tok').catch((cause: unknown) => cause);
  ...
  describe('query parameters reach the server', () => {
    it('passes the expiry window through rather than filtering client-side', async () => {
      ok({ batches: [] });
      await api.stock('tok', 90);

      // Filtering in the browser would mean downloading every batch a pharmacy owns to show
      // the handful expiring soon, and would quietly disagree with the server's own cutoff.
      expect(lastCall().url).toContain('expiringWithinDays=90');
    });

    it('passes the summary date range', async () => {
      ok({});
      await api.salesSummary('tok', '2026-01-01', '2026-03-31');
