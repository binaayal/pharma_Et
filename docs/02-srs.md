...
| FR-4 — POS, standard sale | §5.4 | `mobile/lib/ui/pos_screen.dart`, `mobile/lib/data/sale_repository.dart`, `api/src/modules/sync/sync.service.ts` | `g7_offline_durability_test.dart`, `g2-sync-integrity.spec.ts`, `daily-loop.e2e-spec.ts` (6 journeys) | **Done** — AC-4.1: an offline cash sale is committed locally, queued, and replays exactly once |
| FR-4 — psychotropic dispensing rules | §5.4 | — | — | **Gated on A-1.** AC-4.2 (second psychotropic blocked) and AC-4.3 (prescription older than 15 days rejected) both assert regulatory numbers. Not built, not partially built (ADR-015) |
...
| FR-10 localization | §3 | calendar: `packages/contracts/src/ethiopian-calendar.ts` + `mobile/lib/core/ethiopian_date.dart` (two implementations, one shared vector table) · strings: `mobile/lib/l10n/` · console toggle: `dashboard/src/lib/format.ts` | `ethiopian_date_test.dart` (19), `strings_test.dart` (7), `calendar.spec.ts` (16), `g4-utc-storage.spec.ts` (5) | **Done** — AC-10.1 Amharic + Ethiopian calendar, AC-10.2 UTC storage asserted at the schema level. Corrected 2026-09-24 after a device run: the stock screens, sync chip and expired-stock dialog were English-only (the strings were tested, the screens were not — now `inventory_screens_test.dart` renders them in Amharic), and every expiry date displayed a day early in UTC+3 because a calendar date was read as an instant (`Strings.calendarDate`; CI now runs the mobile suite in the Addis Ababa time zone) |
| Screens & views (T3) | `05-qa` §3 | `mobile/lib/ui/`, `dashboard/src/pages/` | `mobile/test/widget/` (28), `dashboard/test/pages.spec.tsx` (6) | **Done** — §3 asks for critical-path and widget tests on stateful UI, and these pin what is only visible on screen: the cash-up's expected figure hidden until the count is in, the expired-stock warning and who may authorise it, the offline banner, the sync chip telling an ended session apart from an outage, and the 401 contract on every console page. |
...
| NFR-3.1 1,000 tenants | §7, §9, ADR-003 | per-tenant change sequence; an index behind every tenant predicate | `nfr3.perf-spec.ts` (query plans), `nfr3-concurrency.perf-spec.ts` (RLS cost, contention) | Partial — every tenant-scoped table is index-backed and RLS costs +2.5 ms p95 on a scoped read. **Not a 1,000-tenant load test**: that needs a load generator and hosted infrastructure (`06` §11) |
