...
| NFR-4.2 credentials + rate limiting | §8, ADR-017 | `api/src/modules/auth/login-throttle.service.ts`, `migrations/LoginAttempts`, `common/auth/jwt-auth.guard.ts`, `mobile/lib/auth/{session,offline_window}.dart` | `g1-login-throttling.spec.ts` (7), `g1-token-lifecycle.spec.ts` (10), `g7_offline_window_test.dart` (10) | **Done** — throttled never locked out; token expiry **and purpose** enforced (a refresh token no longer authenticates); offline cache tested at the window boundary (`05-qa` §10) |
